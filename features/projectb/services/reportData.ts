...
export interface ReportBrandMetric {
  ...
  sentimentScore: number | null; // null when no observed sentiment
  sentimentLabel: string; // e.g. "+75", "-53", "Neutral", "Unknown"
  avgPosition: number | null; // e.g. 1.08, 1.17 or null
  quadrant: 'Leader' | 'Niche' | 'Low Conversion' | 'Low Performance' | 'Unknown';
  trend: string; // observed trend only; "—" when no comparison exists
}
...
export function buildReportDataModel(
  ...
  const brandRankingUnsorted: ReportBrandMetric[] = Object.keys(brandStats).map((brand) => {
    ...
    // Sentiment is only reported when the extraction produced observed labels.
    ...
    // Quadrant is retained for legacy consumers, but is Unknown without sentiment evidence.
    let quadrant: 'Leader' | 'Niche' | 'Low Conversion' | 'Low Performance' | 'Unknown' = 'Unknown';
    if (sentimentScore !== null) {
      if (coverage >= 10 && sentimentScore >= 90) quadrant = 'Leader';
      else if (coverage >= 10) quadrant = 'Low Conversion';
      else if (sentimentScore >= 80) quadrant = 'Niche';
      else quadrant = 'Low Performance';
    }
