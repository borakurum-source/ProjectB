...
export function GoogleIntegrationCard({ clientDomain, clientBrandName }: GoogleIntegrationCardProps) {
  ...
  const saveSelection = async (site: string, prop: string) => {
    ...
    if (!response.ok) {
      ...
      throw new Error(body.error || `Google property auto-save failed (HTTP ${response.status})`);
    }
  };
  ...
  const fetchStatus = async () => {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(body.error || `Google status request failed (HTTP ${res.status})`);
      }
      ...
      if (autoMatched && (site || prop)) {
        await saveSelection(site, prop);
      }
      setActionError(null);
    } catch (err: any) {
      setActionError(err?.message || 'Failed to load Google Integration status.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleConnectGoogle = async () => {
    ...
    if (!googleState?.clientIdConfigured) {
      setActionError('Google OAuth is not configured in the private runtime. Ask an administrator to configure it.');
      return;
    }
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const body = await response.json().catch(() => ({}));
        throw new Error(body.error || `Google disconnect failed (HTTP ${response.status})`);
      }
      fetchStatus();
    } catch (err: any) {
      setActionError(err?.message || 'Failed to disconnect Google.');
    }
  };
  ...
  const handleSaveConfig = async () => {
    try {
      ...
      if (!response.ok) {
        const body = await response.json().catch(() => ({}));
        throw new Error(body.error || `Google settings save failed (HTTP ${response.status})`);
      }
      setSavedMsg('Google properties updated successfully!');
      setTimeout(() => setSavedMsg(''), 3000);
      fetchStatus();
    } catch (err: any) {
      setActionError(err?.message || 'Failed to save Google settings.');
    }
  };
  ...
  return (
    <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {isConnected ? (
                ...
                <span className="text-[10px] bg-[#EFF6FF] dark:bg-[#1E3A8A] text-[#1D4ED8] dark:text-[#BFDBFE] border border-[#BFDBFE] dark:border-[#1D4ED8] px-1.5 py-0.5 font-bold uppercase tracking-wider inline-flex items-center gap-1">
                  <ShieldCheck className="w-3 h-3" /> CREDENTIALS READY
                </span>
              ) : (
      ...
      <p className="text-xs text-[#6B7280] dark:text-[#94A3B8]">Google OAuth client credentials are administered only through the private runtime configuration and are never displayed or accepted in this browser.</p>
