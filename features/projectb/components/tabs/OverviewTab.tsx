...
import { classifyMeasurement } from '../../services/measurementState';
...
export function OverviewTab({
  ...
  const clientSov = latestCycle?.shareOfVoice?.[client.brandName]?.share ?? null;
  const mentionRate = latestCycle?.overallMentionRate ?? null;
  const citationRate = latestCycle?.overallCitationRate ?? null;
  const volatileCount = latestCycle?.volatilityCount ?? 0;
  const mentionMeasurement = classifyMeasurement({
    totalRuns: totalRunsInLatest,
    value: latestCycle?.overallMentionRate,
    measuredAt: latestCycle?.startedAt,
  });
  const citationMeasurement = classifyMeasurement({
    totalRuns: totalRunsInLatest,
    value: latestCycle?.overallCitationRate,
    measuredAt: latestCycle?.startedAt,
  });

  const topCompetitor = latestCycle
    ? (client.competitorBrands || [])
        .map((comp) => ({
          brand: comp,
        ...
        <div className="bg-[#F8FAFC] dark:bg-[#0F172A] border border-[#CBD5E1] dark:border-[#334155] p-5 text-sm text-[#1E293B] dark:text-[#F8FAFC] flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xs">
          <div className="space-y-1">
            ...
            <p className="text-xs text-[#64748B] dark:text-[#94A3B8]">
              RAGSIGNAL yapay/tahmini veri üretmez. Gemini Grounded motoru her aktif prompt için Google Search grounding ile gerçek zamanlı ölçüm yapar; marka mention&apos;larını ve kaynak domainlerini ayrı ayrı kaydeder.
            </p>
          </div>
          <button
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-4">
        ...
        <div className="bg-white dark:bg-[#0F172A] p-3.5 sm:p-5 border border-[#E5E7EB] dark:border-[#1E293B] shadow-2xs">
          ...
          <div className="text-xl sm:text-2xl font-semibold text-[#111827] dark:text-[#F8FAFC]">
            {mentionMeasurement.display}
          </div>
          <div className="text-[10px] sm:text-[11px] text-[#6B7280] dark:text-[#94A3B8] mt-1 truncate">
            {mentionMeasurement.kind === 'not_measured'
              ? 'Run a measurement cycle to establish a baseline'
              : `${Math.round((mentionRate ?? 0) * totalRunsInLatest)} of ${totalRunsInLatest} runs`}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-[#0F172A] p-3.5 sm:p-5 border border-[#E5E7EB] dark:border-[#1E293B] shadow-2xs">
          ...
          <div className="text-xl sm:text-2xl font-semibold text-[#111827] dark:text-[#F8FAFC]">
            {citationMeasurement.display}
          </div>
          <div className="text-[10px] sm:text-[11px] text-[#6B7280] dark:text-[#94A3B8] mt-1 truncate">
            {citationMeasurement.kind === 'not_measured'
              ? 'Run a measurement cycle to establish a baseline'
              : `Cited in ${Math.round((citationRate ?? 0) * totalRunsInLatest)} runs`}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-[#0F172A] p-3.5 sm:p-5 border border-[#E5E7EB] dark:border-[#1E293B] shadow-2xs">
          ...
          <div className="text-xl sm:text-2xl font-semibold text-[#10B981] dark:text-[#34D399]">
            {clientSov === null ? '—' : `${Math.round(clientSov * 100)}%`}{' '}
            <span className="text-[10px] sm:text-xs font-mono text-[#9CA3AF] dark:text-[#64748B] font-normal">
              (n={totalRunsInLatest})
            </span>
          </div>
          <div className="text-[10px] sm:text-[11px] text-[#6B7280] dark:text-[#94A3B8] mt-1 truncate">
            {topCompetitor ? `Rival: ${topCompetitor.brand} (${Math.round(topCompetitor.share * 100)}%)` : 'Not measured'}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-[#0F172A] p-3.5 sm:p-5 border border-[#E5E7EB] dark:border-[#1E293B] shadow-2xs">
          ...
          <div className="text-xl sm:text-2xl font-semibold text-[#F59E0B] dark:text-[#FBBF24]">
            {latestCycle ? (volatileCount > 0 ? (volatileCount >= 3 ? 'High' : 'Medium') : 'Stable') : 'Not measured'}{' '}
            <span className="text-[10px] sm:text-xs font-mono text-[#9CA3AF] dark:text-[#64748B] font-normal">
              ({latestCycle ? volatileCount : 'n=0'})
            </span>
          </div>
          <div className="text-[10px] sm:text-[11px] text-[#6B7280] dark:text-[#94A3B8] mt-1 truncate">
