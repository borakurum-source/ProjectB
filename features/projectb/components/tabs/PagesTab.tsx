...
export function PagesTab({
  ...
  useEffect(() => {
    setCurrentAnalysis(savedAnalyses[0] || null);
  }, [client.id, savedAnalyses]);
  ...
  const handleCheckCrawlability = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(body.error || `Crawlability check failed (HTTP ${res.status})`);
      }
      const data = await res.json();
      setCrawlData(data);
    } catch (err: any) {
      setActionError(err?.message || 'Crawlability check failed.');
    } finally {
  ...
  const handleCheckSchema = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
        throw new Error(body.error || `Schema check failed (HTTP ${res.status})`);
      }
      const data = await res.json();
      setSchemaData(data);
    } catch (err: any) {
      setActionError(err?.message || 'Schema check failed.');
    } finally {
  ...
  const handleAnalyze = async (e: FormEvent) => {
    ...
    try {
      ...
      setActionError(err?.message || 'Page analysis failed.');
    } finally {
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-[#0F172A] border border-[#E5E7EB] dark:border-[#1E293B] p-5 shadow-xs">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#F3F4F6] dark:border-[#1E293B] pb-4 mb-4">
              ...
              {/* Qualitative extractability status — no synthetic numeric score. */}
              <div className="flex items-center gap-3">
                <div className="text-right">
                  <div className="text-[10px] uppercase tracking-wider text-[#6B7280] dark:text-[#94A3B8] font-bold">Extractability Status</div>
                  <div className="text-lg font-bold text-[#111827] dark:text-[#F8FAFC]">
                    {currentAnalysis.extractabilityStatus || 'Unknown'}
                  </div>
                </div>
                <div className="w-12 h-12 flex items-center justify-center font-mono font-bold text-[10px] text-center uppercase leading-tight border border-[#D1D5DB] dark:border-[#475569] text-[#374151] dark:text-[#CBD5E1] bg-[#F9FAFB] dark:bg-[#1E293B]">
                  {currentAnalysis.extractabilityStatus || 'Unknown'}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div className="p-3.5 bg-[#F9FAFB] dark:bg-[#1E293B] border border-[#E5E7EB] dark:border-[#334155]">
                ...
                <div className="mt-2 text-xs font-bold text-[#111827] dark:text-[#F8FAFC]">
                  {currentAnalysis.hasClearHeadingAnswers ? 'Clear Direct Answers' : 'No Direct Answer Block'}
                </div>
                <div className="text-[11px] text-[#6B7280] dark:text-[#94A3B8] mt-1">
              ...
              {loadingSchema ? (
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-4 border-b border-[#E5E7EB] dark:border-[#1E293B] pb-4">
                    <div className="text-2xl font-black text-[#111827] dark:text-[#F8FAFC]">{schemaData.hasSchema ? 'Present' : 'Missing'}</div>
                    <div>
                      <div className="font-bold text-sm text-[#111827] dark:text-[#F8FAFC]">JSON-LD status</div>
                      <div className="text-xs text-[#6B7280] dark:text-[#94A3B8]">{schemaData.schemaTypes?.length || 0} schema type(s) detected</div>
                    </div>
                  </div>
                  <div>
                    <h4 className="text-xs font-bold uppercase tracking-wider text-[#374151] dark:text-[#CBD5E1] mb-2">Detected Schemas ({schemaData.schemaTypes?.length || 0})</h4>
                    <div className="space-y-2">
                      {schemaData.schemaTypes?.map((type: string, idx: number) => (
                        <div key={idx} className="p-3 border border-[#E5E7EB] dark:border-[#334155] rounded bg-[#F9FAFB] dark:bg-[#1E293B]">
                          <div className="font-mono text-xs font-bold text-[#4338CA] dark:text-[#818CF8] mb-1">{type}</div>
                          <div className="flex items-center gap-2 text-xs mb-1">
                            <span className="font-bold text-[#059669] dark:text-[#34D399]">Detected</span>
                          </div>
                          <div className="text-xs text-[#374151] dark:text-[#CBD5E1]">JSON-LD is present in the fetched page.</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  {!schemaData.hasSchema && (
                    <div>
                      <h4 className="text-xs font-bold uppercase tracking-wider text-[#991B1B] dark:text-[#FCA5A5] mb-2">No JSON-LD detected</h4>
                      <p className="text-xs text-[#7F1D1D] dark:text-[#FCA5A5]">Add only schema types that accurately describe visible page content.</p>
                    </div>
                  )}
                  <div>
                    <h4 className="text-xs font-bold uppercase tracking-wider text-[#374151] dark:text-[#CBD5E1] mb-2">Recommendations</h4>
                    <p className="text-sm text-[#111827] dark:text-[#F8FAFC]">Validate each detected type and keep schema values synchronized with visible content.</p>
                  </div>
                </div>
              ) : null}
            </div>
