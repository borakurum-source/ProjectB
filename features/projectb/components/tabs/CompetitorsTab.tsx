...
import { Sparkles, Smile, Meh, Frown, Globe, LayoutGrid, Table, BarChart2 } from 'lucide-react';
...
export function CompetitorsTab({
  ...
  type CompetitorViewCategory = 'ECOMMERCE' | 'NO ECOMMERCE' | 'UNKNOWN';
  const [categoryFilter, setCategoryFilter] = useState<'ALL' | CompetitorViewCategory>('ALL');

  // Map category for each competitor
  const getCategory = (brand: string, domain: string): CompetitorViewCategory => {
  ...
  const unknownCategoryCount = useMemo(
    () => allCompetitorStats.filter((c) => c.category === 'UNKNOWN').length,
    [allCompetitorStats]
  );
  ...
  const [overlapFilter, setOverlapFilter] = useState<'ALL' | 'COMPETITOR_EXCLUSIVE' | 'SHARED' | 'CLIENT_EXCLUSIVE'>('ALL');

  // Live Competitor Cited Sources Overlap Matrix
  const sourceOverlapAnalysis = useMemo(() => {
    ...
    runs.forEach((r) => {
      ...
      r.groundingSources?.forEach((src) => {
        const d = src.resolvedDomain || src.displayTitle || 'Unresolved source';
        citedDomains.add(d);
      });

      citedDomains.forEach((d) => {
        if (!domainMap[d]) {
          domainMap[d] = {
            domain: d,
            totalCitations: 0,
        ...
        domainMap[d].totalCitations += 1;
        if (r.brandMentioned || r.brandCited) {
          domainMap[d].clientCitations += 1;
        }
        r.mentionedBrands?.forEach((mb) => {
          if (mb.isKnownCompetitor) {
            domainMap[d].competitorCitations[mb.name] =
              (domainMap[d].competitorCitations[mb.name] || 0) + 1;
          }
        });
        const pObj = prompts.find((p) => p.id === r.promptId);
        if (pObj && !domainMap[d].exampleQueries.includes(pObj.text)) {
          if (domainMap[d].exampleQueries.length < 2) domainMap[d].exampleQueries.push(pObj.text);
        }
      });
    });
    ...
    return Object.values(domainMap)
      .map((item) => {
        const hasClient = item.clientCitations > 0;
        const compMentions = Object.keys(item.competitorCitations).length;
        let overlapType: 'Exclusive Competitor Source' | 'Shared / Co-Cited' | 'Client Exclusive Source' =
          'Exclusive Competitor Source';
        if (hasClient && compMentions > 0) overlapType = 'Shared / Co-Cited';
        else if (hasClient && compMentions === 0) overlapType = 'Client Exclusive Source';
        else overlapType = 'Exclusive Competitor Source';
  ...
  const filteredOverlapSources = useMemo(() => {
    if (overlapFilter === 'ALL') return sourceOverlapAnalysis;
    if (overlapFilter === 'COMPETITOR_EXCLUSIVE') {
      return sourceOverlapAnalysis.filter((s) => s.overlapType === 'Exclusive Competitor Source');
    }
    if (overlapFilter === 'SHARED') {
      return sourceOverlapAnalysis.filter((s) => s.overlapType === 'Shared / Co-Cited');
    }
    return sourceOverlapAnalysis.filter((s) => s.overlapType === 'Client Exclusive Source');
  }, [sourceOverlapAnalysis, overlapFilter]);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-[#1E293B] border border-[#E5E7EB] dark:border-[#334155] p-3 flex flex-wrap items-center justify-between gap-3 shadow-xs">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1 bg-[#F3F4F6] dark:bg-[#0F172A] p-0.5 rounded border border-[#E5E7EB] dark:border-[#334155]">
            <button
              ...
              className={`px-3 py-1 text-xs font-bold uppercase tracking-wider transition-colors inline-flex items-center gap-1.5 ${
                viewMode === 'visual'
        ...
        <div className="text-[11px] font-mono text-[#6B7280] dark:text-[#94A3B8]">
          {allCompetitorStats.length} Tracked Competitors • {promptAggregates.length} Prompts
        </div>
      </div>
      ...
      {viewMode === 'visual' ? (
        ...
        <div className="space-y-6">
          {/* 1. Historical Share of Voice Trend Multi-Line Chart */}
          <ShareOfVoiceChart
            cycles={cycles.length > 0 ? cycles : (latestCycle ? [latestCycle] : [])}
          ...
          {/* 2. Flagship Prompt x Brand Presence Heatmap */}
          <CompetitorHeatmap
          ...
          {/* 3. Publisher Citation Density Heatmap */}
          ...
          {/* 4. Brand Sentiment Breakdown */}
          <div className="bg-white dark:bg-[#0F172A] border border-[#E5E7EB] dark:border-[#1E293B] p-5 shadow-xs space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-[#F3F4F6] dark:border-[#1E293B]">
              <div>
                  ...
                  return (
                    <div
                      ...
                      className={`p-3 border rounded-xs space-y-2 ${
                        isClient ? 'bg-[#ECFDF5]/30 border-[#A7F3D0] dark:bg-[#064E3B]/20 dark:border-[#065F46]' : 'bg-[#F9FAFB] border-[#E5E7EB] dark:bg-[#1E293B] dark:border-[#334155]'
                      }`}
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-xs text-[#111827] dark:text-[#F8FAFC]">{brand}</span>
                        {isClient && (
                          <span className="text-[10px] font-bold bg-[#065F46] text-white px-1.5 py-0.2 rounded-xs">CLIENT</span>
                        )}
                      </div>

                      <div className="text-[11px] text-[#6B7280] dark:text-[#94A3B8] flex items-center justify-between font-mono">
                        <span>{s.total} Total Mentions</span>
                        <span className="font-bold text-[#10B981]">{posPct}% Positive</span>
                      </div>

                      {/* Stacked bar ratio */}
                      <div className="w-full h-2.5 bg-[#E5E7EB] dark:bg-[#334155] rounded-full overflow-hidden flex">
                      ...
                      <div className="flex items-center justify-between text-[10px] text-[#4B5563] dark:text-[#94A3B8] pt-1 font-mono">
        ...
        <div className="space-y-6">
                ...
                <button
                  ...
                  className={`px-3 py-1 text-xs font-bold uppercase tracking-wider transition-colors ${
                    categoryFilter === 'UNKNOWN'
                      ...
                      : 'text-[#6B7280] dark:text-[#94A3B8] hover:text-[#475569]'
                  }`}
                >
                  Unknown ({unknownCategoryCount})
                </button>
              </div>
            </div>
            <div className="text-[11px] font-mono text-[#6B7280] dark:text-[#94A3B8]">
                    ...
                    <span
                      className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded-xs shrink-0 tracking-wider font-mono ${
                        comp.category === 'ECOMMERCE'
                          ...
                          : comp.category === 'NO ECOMMERCE'
                            ? isSelected ? 'bg-[#7C3AED] text-white' : 'bg-[#F3E8FF] text-[#6B21A8] border border-[#E9D5FF]'
                            : isSelected ? 'bg-[#475569] text-white' : 'bg-[#F1F5F9] text-[#475569] border border-[#CBD5E1]'
                      }`}
                    >
                      {comp.category === 'UNKNOWN' ? 'UNKNOWN' : comp.category}
                    </span>
                  </div>
          ...
          <div className="bg-white dark:bg-[#0F172A] border border-[#E5E7EB] dark:border-[#1E293B] shadow-xs">
            <div className="p-4 border-b border-[#E5E7EB] dark:border-[#1E293B] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <Globe className="w-4 h-4 text-[#4338CA] dark:text-[#818CF8]" />
                <div>
                  <h3 className="text-xs font-bold uppercase tracking-wider text-[#111827] dark:text-[#F8FAFC]">
                    Competitor Cited Sources Overlap & Digital PR Gap
                  </h3>
                  <p className="text-[11px] text-[#6B7280] dark:text-[#94A3B8]">
                    Discovered publisher domains cited by AI models when recommending competitors vs your brand.
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-1 bg-[#F3F4F6] dark:bg-[#1E293B] p-0.5 rounded border border-[#E5E7EB] dark:border-[#334155]">
                ...
                >
                  All ({sourceOverlapAnalysis.length})
                </button>
                <button
                  onClick={() => setOverlapFilter('COMPETITOR_EXCLUSIVE')}
                  className={`px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider transition-colors ${
                    overlapFilter === 'COMPETITOR_EXCLUSIVE'
                      ? 'bg-[#DC2626] text-white shadow-xs'
                      : 'text-[#6B7280] dark:text-[#94A3B8] hover:text-[#DC2626]'
                  }`}
                >
                  Competitor Gap ({sourceOverlapAnalysis.filter((s) => s.overlapType === 'Exclusive Competitor Source').length})
                </button>
                <button
                  onClick={() => setOverlapFilter('SHARED')}
                  className={`px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider transition-colors ${
                    overlapFilter === 'SHARED'
                      ? 'bg-[#2563EB] text-white shadow-xs'
                      : 'text-[#6B7280] dark:text-[#94A3B8] hover:text-[#2563EB]'
                  }`}
                >
                  Shared ({sourceOverlapAnalysis.filter((s) => s.overlapType === 'Shared / Co-Cited').length})
                </button>
              </div>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs text-left border-collapse">
                ...
                <tbody className="divide-y divide-[#E5E7EB] dark:divide-[#1E293B]">
                  {filteredOverlapSources.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="py-6 text-center text-[#6B7280] dark:text-[#94A3B8]">
                        No citation sources match the selected filter.
                      </td>
                    </tr>
                  ) : (
                    filteredOverlapSources.slice(0, 15).map((src) => {
                      const compList = Object.entries(src.competitorCitations);
                      const isGap = src.overlapType === 'Exclusive Competitor Source';

                      return (
                        <tr key={src.domain} className="hover:bg-[#F9FAFB] dark:hover:bg-[#1E293B] transition-colors">
                          <td className="py-2.5 px-3 font-semibold text-[#111827] dark:text-[#F8FAFC]">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="font-mono text-xs">{src.domain}</span>
                            </div>
                            {src.exampleQueries.length > 0 && (
                              <div className="text-[10px] text-[#6B7280] dark:text-[#94A3B8] mt-0.5 truncate max-w-sm">
                                Query: &quot;{src.exampleQueries[0]}&quot;
                              </div>
                            )}
                          </td>
                          <td className="py-2.5 px-2 text-center font-mono font-bold text-[#111827] dark:text-[#F8FAFC]">
                            {src.totalCitations}
                          </td>
                          <td className="py-2.5 px-3">
                            <div className="flex flex-wrap gap-1">
                              {compList.length === 0 ? (
                                ...
                                compList.map(([compName, count]) => (
                                  <span
                                    key={compName}
                                    className="px-1.5 py-0.5 text-[10px] font-mono rounded bg-[#FEF3C7] dark:bg-[#78350F]/40 text-[#92400E] dark:text-[#FDE68A] border border-[#FDE68A] dark:border-[#B45309]"
                                  >
                                    {compName} ({count})
                                  </span>
                                ))
                          ...
                          <td className="py-2.5 px-2 text-center font-mono">
                            {src.clientCitations > 0 ? (
                              <span className="text-[#166534] dark:text-[#BBF7D0] font-bold text-xs">
                                ✓ {src.clientCitations}x
                              </span>
                            ) : (
                              <span className="text-[#991B1B] dark:text-[#FECACA] font-bold text-xs">
                                0x
                              </span>
                            )}
                          </td>
                            ...
                            <span
                              className={`inline-block px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded ${
                                isGap
                                  ...
                                  : src.overlapType === 'Shared / Co-Cited'
                                  ? 'bg-[#DBEAFE] text-[#1E40AF] dark:bg-[#1E3A8A] dark:text-[#BFDBFE]'
