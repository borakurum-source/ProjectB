...
interface SummaryMetrics {
  latestMention: number | null;
  mentionDelta: number | null;
  latestCitation: number | null;
  citationDelta: number | null;
  peakMention: number | null;
  peakCitation: number | null;
  latestTopComp: number | null;
...
export function MarketTrendsTab({
  ...
  const timeSeriesData: CycleDataPoint[] = useMemo(() => {
    ...
    const dataPoints: CycleDataPoint[] = sortedCycles.map((cycle) => {
      ...
      const totalRuns = cycleRuns.length > 0 ? cycleRuns.length : cycle.totalRuns || 0;
      let mentionCount = 0;
      let citationCount = 0;
      let posSum = 0;
      let posCount = 0;
      ...
      const mentionRate =
        selectedIntent === 'ALL' && cycle.overallMentionRate !== undefined
          ...
          : totalRuns > 0 ? Math.round((mentionCount / totalRuns) * 100) : 0;

      const citationRate =
        selectedIntent === 'ALL' && cycle.overallCitationRate !== undefined
          ? Math.round(cycle.overallCitationRate * 100)
          : totalRuns > 0 ? Math.round((citationCount / totalRuns) * 100) : 0;
      ...
      const topCompetitorRate =
        selectedIntent === 'ALL' && cycle.shareOfVoice?.[topCompName]
          ...
          : totalRuns > 0 ? Math.round((topCompCount / totalRuns) * 100) : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      {timeSeriesData.length === 0 && (
        <div role="status" className="border border-[#CBD5E1] bg-[#F8FAFC] dark:border-[#334155] dark:bg-[#0F172A] p-4 text-xs text-[#475569] dark:text-[#CBD5E1]">
          No measured run cycles yet. Execute a run cycle to populate historical visibility trends.
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="bg-white dark:bg-[#0F172A] border border-[#E5E7EB] dark:border-[#1E293B] p-4 shadow-xs">
          ...
          <div className="mt-2 flex items-baseline justify-between">
            <span className="text-2xl font-bold font-mono text-[#111827] dark:text-[#F8FAFC]">
              {formatPercent(summaryMetrics.latestMention)}
            </span>
            <div className="flex items-center gap-0.5 text-xs font-bold font-mono">
              {summaryMetrics.mentionDelta === null ? (
          ...
          <div className="mt-2 text-[10px] text-[#6B7280] dark:text-[#94A3B8] border-t border-[#F3F4F6] dark:border-[#1E293B] pt-2 flex justify-between font-mono">
            <span>Peak: {formatPercent(summaryMetrics.peakMention)}</span>
        ...
        <div className="bg-white dark:bg-[#0F172A] border border-[#E5E7EB] dark:border-[#1E293B] p-4 shadow-xs">
          ...
          <div className="mt-2 flex items-baseline justify-between">
            <span className="text-2xl font-bold font-mono text-[#065F46] dark:text-[#34D399]">
              {formatPercent(summaryMetrics.latestCitation)}
            </span>
            <div className="flex items-center gap-0.5 text-xs font-bold font-mono">
              {summaryMetrics.citationDelta === null ? (
                <span className="text-[#6B7280]">—</span>
          ...
          <div className="mt-2 text-[10px] text-[#6B7280] dark:text-[#94A3B8] border-t border-[#F3F4F6] dark:border-[#1E293B] pt-2 flex justify-between font-mono">
            <span>Peak: {formatPercent(summaryMetrics.peakCitation)}</span>
        ...
        <div className="bg-white dark:bg-[#0F172A] border border-[#E5E7EB] dark:border-[#1E293B] p-4 shadow-xs">
          ...
          <div className="mt-2 flex items-baseline justify-between">
            <span className="text-2xl font-bold font-mono text-[#111827] dark:text-[#F8FAFC]">
              {formatPercent(summaryMetrics.latestTopComp)}
            </span>
            <span className="text-xs font-bold font-mono text-[#F59E0B]">
              {summaryMetrics.sovDelta === null
                ? '—'
                : summaryMetrics.sovDelta >= 0
                  ? `+${summaryMetrics.sovDelta}% Client Lead`
                  : `${Math.abs(summaryMetrics.sovDelta)}% Comp Gap`}
            </span>
          </div>
          <div className="mt-2 text-[10px] text-[#6B7280] dark:text-[#94A3B8] border-t border-[#F3F4F6] dark:border-[#1E293B] pt-2 flex justify-between font-mono">
            <span>Top Rival: {timeSeriesData[timeSeriesData.length - 1]?.topCompetitorName || '—'}</span>
          </div>
        </div>
