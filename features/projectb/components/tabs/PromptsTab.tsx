...
export function PromptsTab({
  ...
  const handleRunFanout = async (prompt: Prompt) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(body.error || `Fan-out request failed (HTTP ${res.status})`);
      }
      const data = await res.json();
      setFanoutData(data);
    } catch (err: any) {
      setActionError(err?.message || 'Fan-out request failed.');
    } finally {
  ...
  const handleDiscoverPrompts = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
        throw new Error(body.error || `Prompt discovery failed (HTTP ${res.status})`);
      }
      const data = await res.json();
      setDiscoveredPrompts(data.discoveredPrompts || []);
    } catch (err: any) {
      setActionError(err?.message || 'Prompt discovery failed.');
    } finally {
