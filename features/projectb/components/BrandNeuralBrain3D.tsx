...
export function synapsePoint(source: { x: number; y: number }, target: { x: number; y: number }) {
...
export const BrandNeuralBrain3D: React.FC<BrandNeuralBrain3DProps> = ({
  ...
  useEffect(() => {
    ...
    const width = Math.max(320, canvas.parentElement?.clientWidth || 700);
    const height = 480;
    ...
    const render = () => {
      ...
      graph.links.forEach((link) => {
        ...
        // Explicit synapse junction marker: the graph's relationship is
        // visible even when a pulse is between frames or links overlap.
        const marker = synapsePoint({ x: src.px, y: src.py }, { x: tgt.px, y: tgt.py });
        const markerSize = Math.max(2.5, Math.min(5, (src.radius + tgt.radius) / 10));
        ctx.beginPath();
        ctx.moveTo(marker.x, marker.y - markerSize);
        ctx.lineTo(marker.x + markerSize, marker.y);
        ctx.lineTo(marker.x, marker.y + markerSize);
        ctx.lineTo(marker.x - markerSize, marker.y);
        ctx.closePath();
        ctx.fillStyle = isCompetitor ? '#FB7185' : isGsc ? '#38BDF8' : isAi ? '#C084FC' : '#34D399';
        ctx.globalAlpha = 0.9;
        ctx.fill();
        ctx.globalAlpha = 1;
      });
  ...
  }, [graph, items, filterType, selectedItem, onSelectItem, resizeTick]);
  ...
  return (
    <div className="bg-white dark:bg-[#0F172A] border border-[#E5E7EB] dark:border-[#1E293B] p-5 flex flex-col justify-between relative shadow-xs min-h-[520px]">
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto pb-2 mb-3 text-[11px]">
        ...
        >
          All Synapses ({Math.max(0, (graph?.nodes.length || 1) - 1)})
        </button>
        <button
