...
export const OnboardingModal: React.FC<OnboardingModalProps> = ({
  ...
  const handleScanWebsite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Fallback transition to step 2 with default generated values in target language
      const isTr = language.toLowerCase().includes('türk') || language.toLowerCase().includes('tr');
      setCity(isTr ? 'İstanbul' : 'London');
      setShortSummary(isTr 
        ? `${brandName} (${domain}) web sitesi üzerinden kurumsal hizmetler sunmaktadır.`
        : `${brandName} (${domain}) provides specialized products and services.`);
      setProductsServices(isTr
        ? `${brandName} Kurumsal Çözümleri ve Dijital Hizmetler`
        : `${brandName} Services & Digital Solutions`);
      setStep(2);
    } finally {
      setIsScanning(false);
    }
