...
export function AeoStudioTab({ client }: AeoStudioTabProps) {
  ...
  const fetchContents = async () => {
    try {
      ...
      console.warn('Failed to load AEO contents:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGenerate = async () => {
    try {
      ...
      console.error('Error generating AEO content:', err);
    } finally {
      setGenerating(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`/api/aeo-content/${id}`, { method: 'DELETE' });
      setContents(prev => prev.filter(c => c.id !== id));
      if (selectedContent?.id === id) {
        setSelectedContent(contents.find(c => c.id !== id) || null);
      }
    } catch (err) {
