...
export const BrandMemoryTab: React.FC<BrandMemoryTabProps> = ({ client }) => {
  ...
  const handleSyncCrossFunctional = async () => {
    ...
    try {
      ...
      setSyncNotice(`Sync notice: ${err?.message || 'Sync failed.'}`);
    } finally {
  ...
  const handleCrawlAndIndex = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(`Indexing notice: ${errData.error || 'Failed to crawl.'}`);
      }
    } catch (err: any) {
      ...
      <div className="bg-white dark:bg-[#0F172A] border border-[#E5E7EB] dark:border-[#1E293B] p-5 shadow-xs space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-medium text-[#6B7280] dark:text-[#94A3B8]">
                Gemini Embedding 2 Vector Space
              </span>
            </div>
            ...
            <p className="text-xs text-[#6B7280] dark:text-[#94A3B8] max-w-3xl leading-relaxed">
              Bu beyin, markanızın tüm ürünlerini, fiyatlarını, USPs (ayrışma noktalarını) ve rakiplere karşı üstünlüklerini vektörel hafızada saklar. Ayrıca <strong>Google Search Console arama talepleri</strong> ve <strong>AI motoru algı raporları</strong> ile sürekli senkronize olarak AEO/GEO stratejilerini besler.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title="Sync signals from GSC, GA4, and AI Engine runs into Brand Brain"
            >
              <Activity className={`w-3.5 h-3.5 ${syncingCrossFunctional ? 'animate-spin' : ''}`} />
              {syncingCrossFunctional ? 'Syncing Signals...' : 'Sync Cross-Functional Signals'}
            </button>
            <button
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-1">
          ...
          <div className="bg-white dark:bg-[#1E293B] p-3 border border-[#E2E8F0] dark:border-[#334155] rounded">
            ...
            <div className="text-lg font-bold text-[#0F172A] dark:text-[#F8FAFC] mt-0.5">
              {graph?.nodes.length || 0} Nodes ({graph?.links.length || 0} Synapses)
            </div>
          </div>
